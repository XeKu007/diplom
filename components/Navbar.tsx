...
export default function Navbar() {
  ...
  const getRoleLabel = (role: string) => {
    const map: Record<string, string> = {
      ...
      teacher: "Багш",
      parent: "Эцэг/эх",
      admin: "Бүрэн эрхт админ",
      training: "Сургалтын алба",
  ...
  const getProfileLink = (role: string) => {
    ...
    if (role === "admin" || role === "training" || role === "finance")
      return "/admin/profile";
    return "/student/profile";
  };

  const currentUser = {
    name: sessionUser?.name ?? "Хэрэглэгч",
    id: sessionUser?.userId ?? "",
    role: getRoleLabel(sessionUser?.role ?? ""),
    avatar: (sessionUser?.name ?? "Х").charAt(0).toUpperCase(),
    profileLink: getProfileLink(sessionUser?.role ?? ""),
  };

  // Nav links based on role
  const getNavLinks = () => {
    if (isTeacherPage) {
      return [{ label: "Миний хичээлүүд", href: "/teacher/courses" }];
    }
    if (isParentPage) {
      return [{ label: "Оюутны мэдээлэл", href: "/parent" }];
    }
    return [];
  };
  ...
  const handleChangePassword = async () => {
    ...
    if (passwordData.newPassword.length < 8) {
      setPwError("Нууц үг дор хаяж 8 тэмдэгт байх ёстой!");
      return;
    }
    setPwLoading(true);
    try {
      ...
      setPwError("Серверийн алдаа гарлаа.");
    } finally {
      setPwLoading(false);
    }
  };
            ...
            >
              ...
              <div className="p-1.5">
                <button
                  onClick={() => {
                    setOpen(false);
                    router.push(currentUser.profileLink);
                  }}
                ...
                >
                  ...
                  >
                    ...
                    />
                    <path
                      d="M5 6.5V5a2.5 2.5 0 015 0v1.5"
                      stroke="currentColor"
                      strokeWidth="1.2"
                      strokeLinecap="round"
                    />
                ...
                >
                  ...
                  >
                    <path
                      d="M5.5 7.5H13M10.5 5l2.5 2.5-2.5 2.5"
                      stroke="currentColor"
                      strokeWidth="1.2"
                      strokeLinecap="round"
                    ...
                    />
                    <path
                      d="M8 3H3a1 1 0 00-1 1v7a1 1 0 001 1h5"
                      stroke="currentColor"
                      strokeWidth="1.2"
                      strokeLinecap="round"
                    />
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0a1628] shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-white/50">
                  💡 Нууц үг дор хаяж 8 тэмдэгт, том жижиг үсэг, тоо агуулсан
                  байх ёстой
                </p>
              </div>
            ...
            <div className="flex gap-3 border-t border-white/10 bg-[#0a1628] px-6 py-4 rounded-b-2xl">
              ...
              <button
                ...
                disabled={pwLoading}
                className="flex-1 rounded-lg border border-blue-400/40 bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:from-blue-600 hover:to-blue-700 transition-all shadow-lg hover:shadow-blue-500/20 disabled:opacity-60"
              >
                {pwLoading ? "Солиж байна..." : "Солих"}
              </button>
            </div>
          </div>
        </div>
